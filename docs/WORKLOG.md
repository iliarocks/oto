...
## Separate the album source from consumable additions — September 8

Implemented the agreed source/manual distinction, preserving pending additions when choosing another album and consuming active additions when left. Previous and Repeat All follow the source; manual Next and queue jumps start playback and exit Repeat One. Previous resets any nonzero paused position while preserving pause and repeat, including restored or still-loading tracks; track changes start playback and exit Repeat One. Playing restarts retain repeat. The queue UI separates Queued and Next from the source, with Clear limited to pending additions and editing confined to each section. The album button follows source identity rather than the album of a manual detour.

Saved state now records the source cursor and manual additions explicitly. Legacy flat saves retain their current and remaining order without guessing provenance; new album selection establishes an ordinary source. Reconciliation removes unavailable entries while preserving the resume point. Core verification passed all 50 tests (`work/source-queue50-core-final.xcresult`), including playback, repeat boundaries, paused/restored Previous, consumption, source replacement, duplicate entries, editing, restoration, and legacy decoding. Initial failures were the two old pause-preserving skip expectations, which were updated to the agreed behavior. Signed build 1.0 (50) succeeds; UI verification and device delivery follow.
...
## Persistent player, song transitions, and explicit app accent — September 8

Moved the mini-player to a single overlay above NavigationStack. Its measured height supplies per-screen clearances, preserving full access to the last song and larger text without recreating or sliding the player during navigation. Its initial appearance, transport glyphs, and song-speaker indicators remain immediate.

Song changes crossfade Now Playing artwork and metadata over 0.28 seconds while the native queue animates remaining rows into place. Covers and sampled palettes retain their previous values until replacements load, avoiding placeholder/default-color flashes. An interpolated RGB environment tint carries intermediate artwork colors to SwiftUI, the UIKit slider, and AirPlay. Reduce Motion shortens the fades and disables queue movement; the existing artwork-to-queue geometry transition remains intact.

Normal playback/navigation and final-song clearance passed (`work/player52-ui.xcresult`). The updated checks confirm there is one player at the same coordinates across library/album navigation, large-text clearance, and source/manual queue behavior with distinct generated album covers (`work/player52-motion.xcresult`). Reviewed an uninterrupted recording of queue advancement, artwork crossfades, and intermediate accent colors (`work/player52-continuous.mp4`); its source/queue flow also passed. The temporary recording-only waits were removed from the test helper afterward.

The user reported system-blue app accents while this build was in progress. The named asset and build configuration remained correct, but global controls relied on automatic tint inheritance. Set the named light/dark accent explicitly for the app, library controls/swipe action, Settings, and fallback artwork theme, retaining primary-colored navigation icons. Settings, refresh, and source/artwork override checks passed (`work/player52-accent.xcresult`), and screenshots confirmed the Nagare-matching color in Settings and the refresh banner. Final navigation-icon preservation verification and delivery follow.

The final Settings/refresh checks passed (`work/player52-final-accent.xcresult`), with screenshots confirming the named accent on controls and primary-colored gear/close icons. Signed build 1.0 (52) succeeded, its packaged build number was verified, and it was installed and launched normally on the connected iPhone. Existing library and playback data were preserved.

## Contrast-safe artwork swipe actions — September 8

Limited this change to the swipe-action findings from the color audit. Album Add to Queue on the library now samples that row's artwork using the same theme as its album page. All three swipe actions (album add, song add, queue remove) choose black or white ink against their actual artwork fill. Original-rendering SF Symbol images preserve that ink through the native swipe-action rendering, which otherwise templates the icon white. Accessible action names and native swipe/add/remove behavior are retained. Other audit findings remain unchanged.
